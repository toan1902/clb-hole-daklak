  ...
  try{
    ...
    if(location.pathname==='/doanh-nghiep.html'){
      ...
      if(actions){
        ...
        if(site){const a=el('a','Website ↗','business-website');a.href=site;a.target='_blank';a.rel='noopener noreferrer';actions.append(a)}
        const fb=CMS.websiteURL(profile.facebook);
        if(fb){const a=el('a','Facebook ↗','business-website');a.href=fb;a.target='_blank';a.rel='noopener noreferrer';actions.append(a)}
      }
      ...
      if(infoCard){
        ...
        const row=(label,value)=>{if(!value)return;const wrap=el('div',undefined,'info-row');wrap.append(el('strong',label),el('span',value));infoCard.append(wrap)};
        const linkRow=(label,url)=>{if(!url)return;const wrap=el('div',undefined,'info-row'),a=el('a',url.replace(/^https?:\/\//,''));a.href=url;a.target='_blank';a.rel='noopener noreferrer';wrap.append(el('strong',label),a);infoCard.append(wrap)};
        row('Đại diện',profile.representative);
        row('Lĩnh vực',profile.sector);
        row('Điện thoại',profile.phone);
        row('Địa chỉ',profile.address);
        linkRow('Website',CMS.websiteURL(profile.website));
        linkRow('Facebook',CMS.websiteURL(profile.facebook));
        if(!infoCard.children.length)infoCard.append(el('p','Thông tin liên hệ đang được cập nhật.'));
      }
      const bodyBlocks=(p.body||[]).filter(b=>b.type!=='profile');
      const imgBlocks=bodyBlocks.filter(b=>b.type==='img'&&CMS.imageURL(b.src));
      const textBlocks=bodyBlocks.filter(b=>b.type!=='img');
      const contentEl=document.getElementById('content');
      if(textBlocks.length)CMS.renderBody(contentEl,textBlocks);
      else contentEl.replaceChildren(el('p','Thông tin giới thiệu đang được cập nhật.'));
      const gallerySection=document.getElementById('gallerySection'),gallery=document.getElementById('gallery');
      if(gallerySection&&gallery){
        if(imgBlocks.length){
          gallery.replaceChildren();
          for(const b of imgBlocks){
            const fig=el('figure'),img=el('img');img.src=CMS.imageURL(b.src);img.alt=b.caption||p.title;img.loading='lazy';fig.append(img);
            if(b.caption)fig.append(el('figcaption',b.caption));
            gallery.append(fig);
          }
          gallerySection.hidden=false;
        }else gallerySection.hidden=true;
      }
      return;
    }
    if(title){
